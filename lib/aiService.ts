...
const responseSchema = {
  ...
  items: {
    ...
    properties: {
      headword: {
        type: Type.STRING,
        description: 'The word in English (lowercase unless proper noun)',
      },
            ...
            examples: {
              ...
              items: {
                ...
                properties: {
                  ...
                  tgt_roman: {
                    type: Type.STRING,
                    description: 'Romanization of Myanmar translation',
                  },
                },
                required: ['src', 'tgt'],
              },
...
export const searchHuggingFace = async (word: string): Promise<DictionaryEntry | null> => {
  try {
    ...
    const response = await fetch(
      `https://datasets-server.huggingface.co/search?dataset=chuuhtetnaing%2Fenglish-myanmar-dictionary-dataset-mcfnlp&config=default&split=train&query=${query}`
    );
    ...
    if (json.rows && json.rows.length > 0) {
      const exactMatches = json.rows.filter(
        (r: any) => r.row.word.toLowerCase() === word.toLowerCase()
      );
...
export const checkAndCorrectEntry = async (
...
export const enrichEntryExamples = async (
  entry: DictionaryEntry
): Promise<DictionaryEntry | null> => {
  if (!ai) throw new Error('API Key missing');
