...
export const WordGenerator: React.FC = () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (Array.isArray(json)) {
          ...
          alert(
            `Import successful! Added ${result.added} new words and updated ${result.updated} existing words.`
          );
        } else {
  ...
  const inputClass =
    'w-full px-3 py-2 text-sm border border-slate-300 rounded-md focus:border-slate-500 focus:ring-1 focus:ring-slate-200 focus:outline-none bg-white text-slate-900';
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-4 overflow-y-auto pr-1">
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm shrink-0">
            ...
            <div className="space-y-3">
              ...
              <Button onClick={handleGenerate} disabled={isGenerating} className="w-full h-9">
                {isGenerating ? (
                  <>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex-1 overflow-hidden flex flex-col animate-in fade-in">
              <div className="flex justify-between items-center mb-3 shrink-0">
                <h3 className="font-semibold text-purple-900 text-xs">
                  Preview ({generatedEntries.length})
                </h3>
                <Button
                  size="sm"
              ...
              <div className="space-y-2 overflow-y-auto pr-1 custom-scrollbar">
                {generatedEntries.map((entry, idx) => (
                  <div
                    key={idx}
                    className="bg-purple-50 p-2 rounded border border-purple-100 text-xs flex justify-between items-center"
                  >
                    <div>
                      <span className="font-bold text-slate-800 block">{entry.headword}</span>
                      <span className="text-slate-500">
                        {entry.senses[0]?.definition.substring(0, 30)}...
                      </span>
                    </div>
                  </div>
                ))}
                  ...
                  <tr>
                    ...
                    >
                      <div className="flex items-center">
                        Word <SortIcon column="headword" />
                      </div>
                    </th>
                    <th className="p-3 font-semibold text-xs text-slate-500 uppercase tracking-wider">
                      Definition
                    </th>
                    <th
                    ...
                    >
                      <div className="flex items-center justify-center">
                        <ThumbsUp className="w-3.5 h-3.5 mr-1" /> <SortIcon column="upvotes" />
                      </div>
                    </th>
                    <th
                      className="p-3 font-semibold text-xs text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-200 transition-colors w-16 text-center select-none"
                    ...
                    >
                      <div className="flex items-center justify-center">
                        <ThumbsDown className="w-3.5 h-3.5 mr-1" /> <SortIcon column="downvotes" />
                      </div>
                    </th>
                    <th className="p-3 font-semibold text-xs text-slate-500 uppercase tracking-wider w-24 text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                    ...
                    >
                      <td className="p-3 align-top w-1/4">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {entry.senses.map((s, i) => (
                            <span
                              key={i}
                              className="text-[10px] font-serif italic text-slate-500 bg-slate-100 px-1 rounded"
                            >
                              {s.pos}
                            </span>
                          ))}
                      ...
                      <td className="p-3 align-top">
                        {entry.senses.map((sense, i) => (
                          <div
                            key={i}
                            className={`mb-1.5 ${i > 0 ? 'border-t border-slate-100 pt-1.5' : ''}`}
                          >
                            <div className="font-myanmar text-xs text-slate-800 leading-relaxed">
                              {sense.definition}
                            </div>
                            <div className="text-[10px] text-slate-400 mt-0.5">{sense.gloss}</div>
                          </div>
                        ))}
                      </td>
