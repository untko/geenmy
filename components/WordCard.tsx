          ...
          <div className="flex justify-between items-start">
            <div>
              <div className="flex items-baseline flex-wrap gap-3">
                <h2
                  className={`text-2xl font-bold text-slate-900 ${isMyanmarHeadword ? 'font-myanmar leading-tight' : 'font-sans'}`}
                >
                  {entry.headword}
                </h2>
              ...
              <div className="mt-1 flex items-center gap-2">
                <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-400">
                  {entry.source_lang === 'en' ? 'English' : 'Myanmar'} &rarr;{' '}
                  {entry.target_lang === 'en' ? 'English' : 'Myanmar'}
                </span>
              </div>
            </div>
        ...
        <div className="divide-y divide-slate-100">
          {entry.senses.map((sense, index) => (
            <div key={sense.sense_id} className="p-5">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                {sense.tags?.map((tag) => (
                  <Badge
                    key={tag}
                    variant="outline"
                    className="text-slate-500 border-slate-200 text-[10px] px-1.5"
                  >
                    {tag}
                  </Badge>
                ))}
              ...
              {/* Definitions */}
              <div className="mb-3">
                <p className="text-base font-semibold text-slate-800 mb-0.5">{sense.gloss}</p>
                <p
                  className={`text-slate-600 ${entry.target_lang === 'my' ? 'font-myanmar text-base' : 'text-sm'}`}
                >
                  {sense.definition}
                </p>
                {sense.usage_note && (
                  <p className="mt-1 text-xs text-slate-500 italic">Note: {sense.usage_note}</p>
                )}
              </div>
              ...
              {sense.examples.length > 0 && (
                <div className="space-y-2 mt-3">
                  {sense.examples.map((example, exIndex) => (
                    ...
                    >
                      <p
                        className={`text-slate-700 text-sm font-medium ${entry.source_lang === 'my' ? 'font-myanmar' : ''}`}
                      >
                        {example.src}
                      </p>
                      {example.src_roman && (
                        <p className="text-slate-400 text-[10px] font-mono mb-0.5">
                          {example.src_roman}
                        </p>
                      )}

                      <p
                        className={`text-slate-600 mt-0.5 text-sm ${entry.target_lang === 'my' ? 'font-myanmar leading-relaxed' : ''}`}
                      >
                        {example.tgt}
                      </p>
                      {example.tgt_roman && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              ...
              >
                ...
                />
                {stats.downvotes > 0 && (
                  <span className="ml-1.5 text-xs font-medium">{stats.downvotes}</span>
                )}
              </Button>
            </div>
          </div>
      ...
      >
        <div className="space-y-4 flex flex-col items-center py-4">
          ...
          <p className="text-center text-slate-600 text-sm max-w-xs">
            To ensure quality and prevent spam, we require contributors to sign in. It only takes a
            second!
          </p>
          <div className="flex gap-2 w-full justify-center mt-4">
            ...
            <Button onClick={handleLogin} className="bg-slate-900 text-white">
              Log In with Google
            </Button>
          </div>
        </div>
